...
public class Json {
    ...
    private Json get(JsonPath path) {
        var current = element;
    ...
    public Json filter(Predicate<Json> predicate) {
        JsonArray source = element.getAsJsonArray();
        JsonArray target = new JsonArray();

        source.forEach(item -> {
            if (predicate.test(new Json(item))) {
                target.add(item);
            }
        });
    ...
    public Json findMatch(Json expected) {
    ...
    public boolean matches(Json expected) {
    ...
    public Json set(String jsonPath, Json value) {
        this.set(jsonPath, value.element);
        return this;
    }
    public Json set(String jsonPath, int number) {
        this.set(jsonPath, new JsonPrimitive(number));
        return this;
    }
    public Json set(String jsonPath, String string) {
